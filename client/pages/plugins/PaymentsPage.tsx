...
export default function PaymentsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t("payments.newPayment")}</CardTitle>
        </CardHeader>
        <CardContent>
          <form
            ...
            onSubmit={async (e) => {
              ...
              const p = await createPayment({ amount: val, currency, method, reference: reference.trim() });
              setRows((r) => [p, ...r]);
              setAmount("");
              setReference("");
              toast.success("OK");
          ...
          >
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="amount">{t("payments.amount")}</label>
              <Input id="amount" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" required />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">{t("payments.currency")}</label>
            ...
            <div className="space-y-2">
              ...
              <Select value={method} onValueChange={(v) => setMethod(v as Payment["method"]) }>
                ...
                <SelectContent>
                  <SelectItem value="card">{t("payments.method_card")}</SelectItem>
                  <SelectItem value="transfer">{t("payments.method_transfer")}</SelectItem>
                  <SelectItem value="blik">{t("payments.method_blik")}</SelectItem>
                  <SelectItem value="cash">{t("payments.method_cash")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2 md:col-span-2">
              <label className="text-sm font-medium" htmlFor="ref">{t("payments.reference")}</label>
              <Input id="ref" value={reference} onChange={(e) => setReference(e.target.value)} placeholder="FV/2024/004" required />
            </div>
            <div>
              <Button type="submit" className="w-full md:w-auto">{t("payments.submit")}</Button>
            </div>
          </form>
        </CardContent>
      </Card>
