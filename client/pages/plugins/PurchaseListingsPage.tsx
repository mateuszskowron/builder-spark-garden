...
export default function PurchaseListingsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const listingsData = await getPurchaseListings(filters);
  ...
  const loadListings = async () => {
    try {
      setIsLoading(true);
      const data = await getPurchaseListings(filters);
      setListings(data);
    } catch (error) {
      toast({
        title: "Error",
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">
            {t("listings.purchases.title")}
          </h1>
          <p className="text-muted-foreground">
            {t("listings.purchases.description")}
          </p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button>
              ...
              {t("listings.purchases.add")}
            </Button>
          </DialogTrigger>
          <DialogContent className="max-h-96 overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{t("listings.purchases.addTitle")}</DialogTitle>
            </DialogHeader>
                      ...
                      {listing.quantity && (
                        <div className="flex items-center gap-1">
                          <span className="text-muted-foreground">
                            Required: {listing.quantity} {listing.unit}
                          </span>
                        </div>
                      )}
                      <div className="flex items-center gap-1">
